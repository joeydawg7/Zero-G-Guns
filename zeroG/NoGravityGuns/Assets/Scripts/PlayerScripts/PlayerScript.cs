...
public class PlayerScript : MonoBehaviour
{
    ...
    [SerializeField] ParticleSystem speedTraiParticles;
    [SerializeField] ParticleSystem speedIndicationParticles;
    ...
    private void FixedUpdate()
    {
        ...
        //slow down if going 2fast2furious
        LimitVelocity();
    ...
    //TODO: flail the legs? or don't do this. who knows!
    void OnFlail()
    {
    ...
    public PlayerScript Die(DamageType damageType, Guns gunWhoShotYou)
    {
        ...
        if (!isDead)
        {
            ...
            //if we find a target manager treat death differently
            TargetsManager TM = FindObjectOfType<TargetsManager>();

            //pass dead player to a function to zoom in on player and such
            if (TM)
                TM.DeadPlayer(gameObject.transform);

            DeadPlayerScript deadPlayerScript = GameObject.Instantiate(deadPlayerObject).GetComponentInChildren<DeadPlayerScript>();
            deadPlayerScript.OnDeadPlayerSpawn(rb, gameObject);
    ...
    public void SetController(int number, Controller controller)
    {
        ...
        Debug.Log("setting <color=" + hexColorCode + ">" + player.descriptiveName + "</color>" + " to have controller: " + controller.name);
        //}

        ControllerLayoutManager.SetPlayerToGamePlayMaps(player);
    ...
    {      
        ...
        speedTraiParticles = speedTrail.GetComponentInChildren<ParticleSystem>();
    }
    #endregion
