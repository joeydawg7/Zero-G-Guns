...
public class RoundEndCanvasScript : MonoBehaviour
{
        ...
        if (weHaveAWinner)
        {
            foreach (var player in ReInput.players.AllPlayers)
            {
                ...
                if (player.GetButtonDown("Join"))
                {
                    ...
                    Camera.main.transform.root.GetComponent<CameraController>().ResetAllSlowdownEffects();
                }
                ...
                if (timer >= 5f && gameIsOver == false)
                {
                    ...
                    {
                        RoundManager.Instance.NewRound(false, true);
                    }
                }
    ...
    private PlayerDataScript getWinningPlayerData(PlayerScript winningPlayer)
    {
        ...
        foreach (var pd in GameObject.FindObjectsOfType<PlayerDataScript>())
        {
    ...
    private void SortAndDisplayRoundWins()
    {
        ...
        List<PlayerDataScript> SortedListOfPlayerDatas = GameObject.FindObjectsOfType<PlayerDataScript>().OrderByDescending(o=>o.roundWins).ToList();
        //RoundManager.Instance.playerDatas.OrderByDescending(o => o.roundWins).ToList();
        ...
        Debug.Log("SORTED DATAS: " + SortedListOfPlayerDatas.Count);
        ...
        for (int i = 0; i < SortedListOfPlayerDatas.Count; i++)
        {
            ...
            {
                Debug.LogError("SortedListOfPlayerDatas[i] is null!");
            }
    ...
    public string GetWittyCommentOnLastHitPoint(PlayerScript.DamageType damageType, Guns gunWhoShotYou)
    {
        ...
        if (options.Length <= 1)
        {
            if (damageType == PlayerScript.DamageType.head)
            {
                ...
                options = new[] { "In the face!", "Oh no not in the brain", "Helmets only do so much", "Bullets and your head a deadly combination", "Headshot!" };
